...
function VideoComment({ id }) {
    ...
    useEffect(() => {
        FetchApi(`commentThreads?part=snippet&videoId=${id}`)
            .then((comments) => setComments(comments.data.items))
        ...
        <div className=' flex flex-col align-middle h-auto m-1 text-xs md:text-sm'>
            {
                comments.length > 0 && comments.map((val) => {
                    return (
                        <div className='p-2 flex flex-col align-middle h-auto bg-indigo-400 shadow-sm shadow-slate-600 rounded-xl m-1' key={val.id}>

                            <div className=' m-1 flex flex-row align-middle'>
                                <img
                                    src={val.snippet.topLevelComment.snippet.
                                        authorProfileImageUrl}
                                    alt="n/a" className=' rounded-full w-4 m-1' />
                            ...
                            <div className='p-1 flex flex-row align-middle justify-between  m-1'>
                                ...
                                <p>{val.snippet.topLevelComment.snippet.publishedAt.substring(0, 10)}</p>
                            </div>
